...
# 用法: skills-update.ps1 [owner/repo](缺省读 ~/.config/skills.txt 全部)
Set-Location $HOME
...
if ($args.Count -gt 0) {
    ...
    $pkgs = Get-Content $Manifest | Where-Object { $_ -and -not $_.TrimStart().StartsWith('#') }
}
